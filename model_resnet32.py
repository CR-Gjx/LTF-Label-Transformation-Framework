...
class SA_Generator32(nn.Module):
    def __init__(self, code_dim=100, n_class=100,nc=3, SN=True):
        super().__init__()
        if SN == True:
            self.lin_code = spectral_init(nn.Linear(code_dim, 4 * 4 * 256))
            self.embedding = spectral_init(nn.Embedding(n_class, 256))
        else:
            self.lin_code = nn.Linear(code_dim, 4 * 4 * 256)
            self.embedding = nn.Embedding(n_class, 256)



        self.conv = nn.ModuleList([ConvBlock(256, 256, n_class=n_class,SN=SN,emb = self.embedding),
                                   # ConvBlock(256, 256, n_class=n_class,SN=SN),
                                   # SelfAttention(256, SN=SN),
                                   ConvBlock(256, 256, n_class=n_class, SN=SN,emb = self.embedding),
                                   ConvBlock(256, 256, n_class=n_class,SN=SN,emb = self.embedding)])

        self.bn = nn.BatchNorm2d(256)
        if SN == True:
            self.colorize = spectral_init(nn.Conv2d(256, nc, [3, 3], padding=1))
        else:
            self.colorize = nn.Conv2d(256, nc, [3, 3], padding=1)
...
class SA_Discriminator32(nn.Module):
    def __init__(self, n_class=100,nc=3,SN=True):
        super().__init__()
        ...
        if SN == True:
            self.pre_conv = nn.Sequential(spectral_init(nn.Conv2d(nc, 256, 3,
                                                                  padding=1),
                                                        gain=gain),
                                          nn.ReLU(),
                                          spectral_init(nn.Conv2d(256, 256, 3,
                                                                  padding=1),
                                                        gain=gain),
                                          nn.AvgPool2d(2))
            self.pre_skip = spectral_init(nn.Conv2d(nc, 256, 1))
        else:
            self.pre_conv = nn.Sequential(nn.Conv2d(nc, 256, 3,padding=1),
                                          nn.ReLU(),
                                          nn.Conv2d(256, 256, 3,padding=1),
                                          nn.AvgPool2d(2))
            self.pre_skip = nn.Conv2d(nc, 256, 1)

        self.conv = nn.Sequential(conv(256, 256,SN=SN),
                                  # conv(256, 256, SN=SN),
                                  # SelfAttention(256, SN=SN),
                                  conv(256, 256,SN=SN),
                                  conv(256, 256,SN=SN))
        if SN == True:
            self.linear = spectral_init(nn.Linear(256, 1))

            self.embed = spectral_norm(nn.Embedding(num_embeddings=n_class, embedding_dim=256))

        else:
            self.linear = nn.Linear(256, 1)

            self.embed = nn.Embedding(num_embeddings=n_class, embedding_dim=256)
