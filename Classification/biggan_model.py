...
class BigD_feature_extractor(nn.Module):
    def __init__(self,D):
        super(BigD_feature_extractor, self).__init__()
        self.D = D

    def forward(self, x):
        h = x
        # Loop over blocks
        for index, blocklist in enumerate(self.D.blocks):
            for block in blocklist:
                h = block(h)
        # Apply global sum pooling as in SN-GAN
        h = torch.sum(self.D.activation(h), [2, 3])
...
class G_D(nn.Module):
    ...
    def forward(self, z, gy, x=None, dy=None, train_G=False, return_G_z=False,
        ...
        if split_D:
            D_fake = self.D(G_z, gy)[0]
            if x is not None:
                D_real = self.D(x, dy)[0]
        ...
        # along the batch dimension for improved efficiency.
            ...
            D_out = self.D(D_input, D_class)[0]
            if x is not None:
